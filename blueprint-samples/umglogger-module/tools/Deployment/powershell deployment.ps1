$env:VersionMinor = %dep.UmgEnterpriseServicesBuild_MuleSoftSamples_ApiGateway_BuildApiGateway.build.counter%
...
#CREATE DEPLOYMENT
$CreateDeploymentResultJson = .\curl.exe --% --basic -u admin:admin -d "{\"name\" : \"umglogger-module%VersionMinor%\" ,\"servers\": [ \"%ServersEnv%\" ],\"applications\": [ \"%UploadedVersionEnv%\" ]}" --header "Content-Type: application/json" %URL%deployments
...
$DeployCommand = .\curl.exe --% --basic -u admin:admin -X POST http://172.25.172.240:8888/mmc351/api/deployments/%DeploymentIdEnv%/deploy
